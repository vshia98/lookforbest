<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-xl font-bold text-gray-800">厂商管理</h2>
      <button
        @click="openForm(null)"
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          >
            ...
            <td class="py-3 px-4">
              <div class="flex items-center gap-3">
                ...
                />
                <div v-else class="w-8 h-8 bg-gray-100 rounded flex items-center justify-center text-gray-400 text-xs">🏭</div>
                <span class="font-medium text-gray-800">{{ m.name }}</span>
              </div>
            </td>
            <td class="py-3 px-4 text-gray-600">{{ m.nameEn || '-' }}</td>
            <td class="py-3 px-4 text-gray-600">{{ m.country || '-' }}</td>
            <td class="py-3 px-4">
              <a v-if="m.websiteUrl" :href="m.websiteUrl" target="_blank" class="text-primary-500 hover:underline text-xs">
                访问官网
              </a>
      ...
      <div v-if="total > pageSize" class="flex items-center justify-between px-4 py-3 border-t border-gray-100">
        ...
        <div class="flex items-center gap-2">
          <button :disabled="page <= 1" @click="page--; loadManufacturers()" class="px-3 py-1 text-sm border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors">上一页</button>
          <span class="text-sm text-gray-500">{{ page }} / {{ totalPages }}</span>
          <button :disabled="page >= totalPages" @click="page++; loadManufacturers()" class="px-3 py-1 text-sm border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors">下一页</button>
        </div>
      </div>
    </div>
    ...
    >
      <div class="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
        <h3 class="font-bold text-gray-800 mb-5">{{ editTarget ? '编辑厂商' : '添加厂商' }}</h3>
        <form @submit.prevent="submitForm" class="space-y-4">
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-sm text-gray-600 mb-1">中文名称 <span class="text-red-500">*</span></label>
              <input v-model="formData.name" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">英文名称</label>
              <input v-model="formData.nameEn" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">国家/地区</label>
              <input v-model="formData.country" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">官网URL</label>
              <input v-model="formData.websiteUrl" type="url" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="https://..." />
            </div>
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">Logo URL</label>
            <input v-model="formData.logoUrl" type="url" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="https://..." />
          </div>
          <p v-if="formError" class="text-red-500 text-sm">{{ formError }}</p>
          <div class="flex gap-3 justify-end pt-2">
            <button type="button" @click="showForm = false" class="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">取消</button>
            <button type="submit" :disabled="formSubmitting" class="px-4 py-2 text-sm bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors disabled:opacity-50">
...
async function loadManufacturers() {
  loading.value = true
  try {
    const res = await manufacturerService.getList({ page: page.value, size: pageSize, q: query.value || undefined })
    manufacturers.value = res.data || res.items || []
    total.value = res.total || manufacturers.value.length
  } catch (e) {
    console.error(e)
  } finally {
    loading.value = false
